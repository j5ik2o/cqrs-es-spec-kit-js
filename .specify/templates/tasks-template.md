...
**テスト**: テストタスクは必須。各ユースケースの単体・契約・統合テストを先に追加し、実装前に失敗させる。
...
- [ ] T010 [P] [US1] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T011 [P] [US1] ポート契約テストを `tests/contract/` に追加する
- [ ] T012 [P] [US1] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T013 [P] [US1] ドメインモデルを `src/domain/...` に追加する
- [ ] T014 [P] [US1] ユースケースサービスを `src/application/use-cases/...` に実装する
- [ ] T015 [US1] アダプタを `src/interfaces/...` に実装し、ポートを満たす
- [ ] T016 [US1] バリデーション・エラーハンドリングを組み込む
- [ ] T017 [US1] 観測性（ログ・メトリクス）を追加する
...
- [ ] T018 [P] [US2] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T019 [P] [US2] 契約テストを `tests/contract/` に追加する
- [ ] T020 [P] [US2] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T021 [P] [US2] 必要なドメインオブジェクトを拡張する
- [ ] T022 [US2] ユースケースサービスを追加・調整する
- [ ] T023 [US2] インターフェイスアダプタを実装し、US1 との衝突を確認する
- [ ] T024 [US2] 監査ログや追跡情報を更新する
...
- [ ] T025 [P] [US3] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T026 [P] [US3] 契約テストを `tests/contract/` に追加する
- [ ] T027 [P] [US3] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T028 [P] [US3] ドメインモデル・ユースケースを拡張する
- [ ] T029 [US3] インターフェイスアダプタを実装する
- [ ] T030 [US3] 監視・メトリクスを更新する
