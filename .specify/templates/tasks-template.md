...
- [ ] T018 [P] [US1] ドメイン単体テストを `tests/unit/` に追加する（回復可能エラーと正常系を検証）
- [ ] T019 [P] [US1] ポート契約テストを `tests/contract/` に追加する（例外変換を含む）
- [ ] T020 [P] [US1] 統合テストを `tests/integration/` に追加する（外部エラー伝播と代替フローを検証）

### 実装

- [ ] T021 [P] [US1] ドメインモデルを `src/domain/...` に追加する
- [ ] T022 [P] [US1] ユースケースサービスを `src/application/use-cases/...` に実装する（Either/Result によるエラー制御を含む）
- [ ] T023 [US1] アダプタを `src/interfaces/...` に実装し、ポートを満たす（例外をドメイン向けエラーに変換し、RPC/DB/イベントストア連携など Gateway 責務を担う）
- [ ] T024 [US1] バリデーション・エラーハンドリングを組み込む（回復不能エラーはフェイルファスト）
- [ ] T025 [US1] 観測性（ログ・メトリクス）を追加する
- [ ] T026 [US1] ドメインサービスが純粋関数であること、およびリファレンスコードをコピーしていないことをレビューする
- [ ] T027 [US1] コマンドハンドラがイベント保存を主目的に設計され、必要最小限のリプレイ・他集約確認に限定して読み込みを行っていることを確認する
- [ ] T028 [US1] バリデータが値オブジェクトのコンストラクタ経由で検証し、成功時は値オブジェクトを返し失敗時はエラーを返していることを確認する
- [ ] T029 [US1] GraphQL スキーマにユースケース専用 Mutation/Query/Subscription を追加し、ドメインサービスと結線する
- [ ] T030 [US1] Next.js API Route を実装し、UI からの入力検証・GraphQL 呼び出し・レスポンス整形を行う
- [ ] T031 [US1] Next.js UI コンポーネントを実装し、BFF 経由でデータ取得・GraphQL サブスクリプション受信・エラー表示を行う
...
- [ ] T032 [P] [US2] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T033 [P] [US2] 契約テストを `tests/contract/` に追加する
- [ ] T034 [P] [US2] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T035 [P] [US2] 必要なドメインオブジェクトを拡張する
- [ ] T036 [US2] ユースケースサービスを追加・調整する（エラー戻り値を拡張）
- [ ] T037 [US2] インターフェイスアダプタを実装し、US1 との衝突を確認する（例外変換と通知。RPC/DB/イベントストア連携はこちらで実装）
- [ ] T038 [US2] 監査ログや追跡情報を更新する（エラー計測を含む）
- [ ] T039 [US2] ドメインサービスの純粋性をテストし、永続化依存やリファレンスコードのコピーが混入していないことを検証する
- [ ] T040 [US2] コマンドハンドラの読み込みがリプレイ/他集約確認など最小限の目的に限定されていることをレビューし、クエリ責務へ逸脱していないことを確認する
- [ ] T041 [US2] バリデータが値オブジェクトのコンストラクタ経由で検証し、成功時は値オブジェクトを返し失敗時はエラーを返していることを確認する
- [ ] T042 [US2] GraphQL スキーマ/リゾルバを拡張し、新しいユースケースを Mutation/Query/Subscription に反映する
- [ ] T043 [US2] Next.js API Route を拡張し、追加データ型・エラーを BFF で吸収する
- [ ] T044 [US2] Next.js UI を拡張し、リアルタイム更新や UX 変更を実装する
...
- [ ] T045 [P] [US3] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T046 [P] [US3] 契約テストを `tests/contract/` に追加する
- [ ] T047 [P] [US3] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T048 [P] [US3] ドメインモデル・ユースケースを拡張する（新しいエラー型を定義）
- [ ] T049 [US3] インターフェイスアダプタを実装する（外部エラーをマッピングし、RPC/DB/イベントストア連携を提供）
- [ ] T050 [US3] 監視・メトリクスを更新する（エラー指標を追加）
- [ ] T051 [US3] ドメインサービスで外部依存が追加されていないか、リファレンスコードがコピーされていないかを静的解析・レビューで確認する
- [ ] T052 [US3] コマンドフロー全体をリプレイし、読み込みが許容範囲に収まっていることとクエリ責務へ逸脱していないことを検証する
- [ ] T053 [US3] バリデータが値オブジェクトのコンストラクタ経由で検証し、成功時は値オブジェクトを返し失敗時はエラーを返していることを確認する
- [ ] T054 [US3] GraphQL リゾルバ・スキーマを拡張し、サブスクリプションや Mutation の追加分を反映する
- [ ] T055 [US3] Next.js API Route を更新し、新しいユースケースの入力検証・レスポンス整形・エラーマッピングを追加する
- [ ] T056 [US3] Next.js UI を更新し、BFF 経由での新しい体験やリアルタイム更新を実装・テストする
