...
- [ ] T011 エラーハンドリング方針をドキュメントに反映し、回復可能/不能の分類と戻り値型・例外境界を明記する
...
- [ ] T012 [P] [US1] ドメイン単体テストを `tests/unit/` に追加する（回復可能エラーと正常系を検証）
- [ ] T013 [P] [US1] ポート契約テストを `tests/contract/` に追加する（例外変換を含む）
- [ ] T014 [P] [US1] 統合テストを `tests/integration/` に追加する（外部エラー伝播と代替フローを検証）

### 実装

- [ ] T015 [P] [US1] ドメインモデルを `src/domain/...` に追加する
- [ ] T016 [P] [US1] ユースケースサービスを `src/application/use-cases/...` に実装する（Either/Result によるエラー制御を含む）
- [ ] T017 [US1] アダプタを `src/interfaces/...` に実装し、ポートを満たす（例外をドメイン向けエラーに変換し、RPC/DB 連携など Gateway 責務を担う）
- [ ] T018 [US1] バリデーション・エラーハンドリングを組み込む（回復不能エラーはフェイルファスト）
- [ ] T019 [US1] 観測性（ログ・メトリクス）を追加する
...
- [ ] T020 [P] [US2] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T021 [P] [US2] 契約テストを `tests/contract/` に追加する
- [ ] T022 [P] [US2] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T023 [P] [US2] 必要なドメインオブジェクトを拡張する
- [ ] T024 [US2] ユースケースサービスを追加・調整する（エラー戻り値を拡張）
- [ ] T025 [US2] インターフェイスアダプタを実装し、US1 との衝突を確認する（例外変換と通知。RPC/DB 連携はこちらで実装）
- [ ] T026 [US2] 監査ログや追跡情報を更新する（エラー計測を含む）
...
- [ ] T027 [P] [US3] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T028 [P] [US3] 契約テストを `tests/contract/` に追加する
- [ ] T029 [P] [US3] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T030 [P] [US3] ドメインモデル・ユースケースを拡張する（新しいエラー型を定義）
- [ ] T031 [US3] インターフェイスアダプタを実装する（外部エラーをマッピングし、RPC/DB 連携を提供）
- [ ] T032 [US3] 監視・メトリクスを更新する（エラー指標を追加）
