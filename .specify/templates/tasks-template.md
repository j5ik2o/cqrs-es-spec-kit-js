...
- [ ] T014 [P] [US1] ドメイン単体テストを `tests/unit/` に追加する（回復可能エラーと正常系を検証）
- [ ] T015 [P] [US1] ポート契約テストを `tests/contract/` に追加する（例外変換を含む）
- [ ] T016 [P] [US1] 統合テストを `tests/integration/` に追加する（外部エラー伝播と代替フローを検証）

### 実装

- [ ] T017 [P] [US1] ドメインモデルを `src/domain/...` に追加する
- [ ] T018 [P] [US1] ユースケースサービスを `src/application/use-cases/...` に実装する（Either/Result によるエラー制御を含む）
- [ ] T019 [US1] アダプタを `src/interfaces/...` に実装し、ポートを満たす（例外をドメイン向けエラーに変換し、RPC/DB/イベントストア連携など Gateway 責務を担う）
- [ ] T020 [US1] バリデーション・エラーハンドリングを組み込む（回復不能エラーはフェイルファスト）
- [ ] T021 [US1] 観測性（ログ・メトリクス）を追加する
- [ ] T022 [US1] ドメインサービスが純粋関数であることを確認し、外部依存がないことをレビューする
...
- [ ] T022 [P] [US2] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T023 [P] [US2] 契約テストを `tests/contract/` に追加する
- [ ] T024 [P] [US2] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T025 [P] [US2] 必要なドメインオブジェクトを拡張する
- [ ] T026 [US2] ユースケースサービスを追加・調整する（エラー戻り値を拡張）
- [ ] T027 [US2] インターフェイスアダプタを実装し、US1 との衝突を確認する（例外変換と通知。RPC/DB/イベントストア連携はこちらで実装）
- [ ] T028 [US2] 監査ログや追跡情報を更新する（エラー計測を含む）
- [ ] T029 [US2] ドメインサービスの純粋性をテストし、永続化依存が混入していないことを検証する
...
- [ ] T029 [P] [US3] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T030 [P] [US3] 契約テストを `tests/contract/` に追加する
- [ ] T031 [P] [US3] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T032 [P] [US3] ドメインモデル・ユースケースを拡張する（新しいエラー型を定義）
- [ ] T033 [US3] インターフェイスアダプタを実装する（外部エラーをマッピングし、RPC/DB/イベントストア連携を提供）
- [ ] T034 [US3] 監視・メトリクスを更新する（エラー指標を追加）
- [ ] T035 [US3] ドメインサービスで外部依存が追加されていないかを静的解析・レビューで確認する
