...
- [ ] T025 [US1] バリデータが値オブジェクトのコンストラクタ経由で検証し、成功時は値オブジェクトを返し失敗時はエラーを返していることを確認する
...
- [ ] T026 [P] [US2] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T027 [P] [US2] 契約テストを `tests/contract/` に追加する
- [ ] T028 [P] [US2] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T029 [P] [US2] 必要なドメインオブジェクトを拡張する
- [ ] T030 [US2] ユースケースサービスを追加・調整する（エラー戻り値を拡張）
- [ ] T031 [US2] インターフェイスアダプタを実装し、US1 との衝突を確認する（例外変換と通知。RPC/DB/イベントストア連携はこちらで実装）
- [ ] T032 [US2] 監査ログや追跡情報を更新する（エラー計測を含む）
- [ ] T033 [US2] ドメインサービスの純粋性をテストし、永続化依存やリファレンスコードのコピーが混入していないことを検証する
- [ ] T034 [US2] コマンドハンドラの読み込みがリプレイ/他集約確認など最小限の目的に限定されていることをレビューし、クエリ責務へ逸脱していないことを確認する
- [ ] T035 [US2] バリデータが値オブジェクトのコンストラクタ経由で検証し、成功時は値オブジェクトを返し失敗時はエラーを返していることを確認する
...
- [ ] T036 [P] [US3] ドメイン単体テストを `tests/unit/` に追加する
- [ ] T037 [P] [US3] 契約テストを `tests/contract/` に追加する
- [ ] T038 [P] [US3] 統合テストを `tests/integration/` に追加する

### 実装

- [ ] T039 [P] [US3] ドメインモデル・ユースケースを拡張する（新しいエラー型を定義）
- [ ] T040 [US3] インターフェイスアダプタを実装する（外部エラーをマッピングし、RPC/DB/イベントストア連携を提供）
- [ ] T041 [US3] 監視・メトリクスを更新する（エラー指標を追加）
- [ ] T042 [US3] ドメインサービスで外部依存が追加されていないか、リファレンスコードがコピーされていないかを静的解析・レビューで確認する
- [ ] T043 [US3] コマンドフロー全体をリプレイし、読み込みが許容範囲に収まっていることとクエリ責務へ逸脱していないことを検証する
- [ ] T044 [US3] バリデータが値オブジェクトのコンストラクタ経由で検証し、成功時は値オブジェクトを返し失敗時はエラーを返していることを確認する
