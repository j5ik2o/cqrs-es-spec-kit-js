...
| ツール | バージョン | 説明 |
|--------|------------|------|
| **Node.js** | v20.5.0 以上 | JavaScript ランタイム（LTS 推奨） |
| **pnpm** | v8 以上 | パッケージマネージャー（プロジェクトは v10.22.0 を使用） |
| **Docker** | 最新版 | コンテナランタイム |
| **Docker Compose** | v2 以上 | コンテナオーケストレーション |
| **Git** | 最新版 | バージョン管理（サブモジュール用） |
...
| **jq** | レスポンス解析用 JSON プロセッサー |

#### オプション

| ツール | 説明 |
|--------|------|
| **AWS CLI** | LocalStack への直接操作用 |
